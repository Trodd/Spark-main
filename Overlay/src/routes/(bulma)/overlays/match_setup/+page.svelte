<script lang="ts">
	...
	function manualClickHandler() {
		...
		httpPostAsync(
			'http://localhost:6724/api/set_team_details/orange',
			{
				team_logo: config['teams'][1]['team_logo'],
				team_name: config['teams'][1]['team_name']
			},
			() => {
				// After update completes, mark as clean so config can update from server
				dirty = false;
			}
		);
	}
...
<div class="content" style="max-width: 60em; margin: auto; position:relative;">
	...
	<div class="box" style="position: relative; top: -2em;font-size: 1.5em;">
		<div class="match_setup_flex">
					...
					<tbody>
						<tr>
							...
							<td style="padding: 3em 0;">
								...
								>
									<img
										src="/img/swap-horizontal-bold.png"
						...
						<tr>
							<td colspan="5"
								><label class="checkbox" style="margin-left: 3em;">
									<input
										checked={config['team_names_source'] === 1}
										type="checkbox"
										on:click={(e) => {
											setTeamNamesSource(e.target.checked);
										}}
									/>
			...
			<div
				class="button is-dark echobutton small {dirty ? '' : 'hide'}"
				id="update_manual_values"
				style="margin:auto; display: block; width: 15em;"
				on:click={manualClickHandler}
			>
				Update
			</div>
		</div>
...
<style>
	...
	.orange.textarea:focus {
		box-shadow: 0 0 0 0.125em rgba(209, 84, 0, 0.25);
	}
	...
	.blue.textarea:focus {
		box-shadow: 0 0 0 0.125em rgba(0, 63, 209, 0.25);
	}
